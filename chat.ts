...
export const Chat = (): Chat => {
    ...
    const isConnected = (socket: Socket): boolean => {
        ...
        for(let i = 0; i < users.length; i++) if(users[i].socket.id == socket.id) return true;
        return false;
    }
    ...
    const getUser = (socket: Socket): User => {
        if(verifyUser(socket)) {
            for(let i = 0; i < users.length; i++) if(users[i].socket.id == socket.id) return users[i];
        }
    ...
    const getOnlineUsers = (socket: Socket = null): Array<OnlineUsers> => {
        ...
        if(socket == null) {
            for(let i = 0; i < users.length; i++) {
                connectedUsers.push({name: users[i].name, socketId: users[i].socket.id})
            }
        }else {
            for(let i = 0; i < users.length; i++) {
                if(users[i].socket.id != socket.id)
